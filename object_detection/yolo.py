...
def get_bounding_boxes(frame, model, conf=0.6, iou=0.8):
    ...
    Returns:
        Processed frame with bounding boxes, detection results
    """
    # Use tracker with improved parameters
    results = model.track(
        source=frame,
        conf=conf,            # Confidence threshold
        iou=iou,              # IoU threshold for NMS
        persist=True,         # Maintain tracking through occlusions
        vid_stride=1,         # Process every frame
        verbose=False,        # Disable console output
        classes=None,         # Detect all classes
        augment=False,        # No augmentation
        agnostic_nms=True,    # Class-agnostic NMS
        max_det=3,            # Maximum detections per frame
        retina_masks=False    # No masks needed
    )
    
    # No need to draw boxes here - will be done by smoothing algorithm
    # Just return the frame and results
